...
async function startSock() {
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        const keywords = ['zyon', 'ia', 'bot'];
        const containsKeyword = keywords.some(keyword => lowerText.includes(keyword));

        if (!containsKeyword) {
            console.log(`[${BOT_NAME}] Mensaje ignorado (no contiene palabra clave).`);
            return; // No responde
        }

        // Si contiene palabra clave, procesa con la IA
        try {
